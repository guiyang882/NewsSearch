...
function delete_user_info(obj) {
    ...
    var content = {"operator":"consumers-deleted","name":single_data.name,"wx_id":single_data.wx_id,"phone":single_data.phone};
...
function save_change_user_info() {
    ...
    if(b_Same == false) {
        //post data to server
        var update_info = {"name":new_user_info[0],"wx_id":new_user_info[1],"phonenum":new_user_info[2],"other_info":new_user_info[3],"status":new_user_info[4],"comments":new_user_info[5]};
        var old_info = {"name":data_list[0],"wx_id":data_list[1],"phonenum":data_list[2],"other_info":data_list[3],"status":data_list[4],"comments":data_list[5]};
        var content = {"operator":"UpdateUserInfo","old_info":JSON.stringify(old_info),"update_info":JSON.stringify(old_info)};
        send_Data_Backend(content);
